  ...
  return (
    <ScreenLayout>
      ...
      <header className="flex items-center justify-between py-4 animate-slide-up">
        <div>
          <p className="text-forza-red/50 text-xs">Good morning</p>
          <h1 className="font-display text-2xl font-bold text-forza-red tracking-wide">
        ...
        >
          <span className="text-forza-red/75 text-sm font-bold">{USER.avatar}</span>
        </button>
      </header>

      {/* Membership card */}
      <section className="relative rounded-2xl overflow-hidden mb-5 card-glow animate-slide-up" style={{ animationDelay: '0.05s' }}>
        <div className="absolute inset-0 bg-gradient-to-br from-forza-red/15 via-forza-white to-forza-white" />
        ...
        <div className="relative p-5 border border-forza-red/20 rounded-2xl">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-forza-red text-xl font-bold mt-1">{tier.name}</p>
              <p className="text-forza-red/60 text-xs">{USER.memberId}</p>
            </div>
            {subscription.isFrozen ? (
              <span className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-forza-red/10 text-forza-red/75 text-[10px] font-medium border border-forza-red/25">
          ...
          <div className="mt-5 grid grid-cols-2 gap-3">
            <div className="bg-forza-red/5 rounded-xl p-3">
              <p className="text-forza-red/50 text-[10px] uppercase tracking-wider">Start</p>
              <p className="text-forza-red text-sm font-semibold mt-0.5">{formatDate(subscription.startDate)}</p>
            </div>
            <div className="bg-forza-red/5 rounded-xl p-3">
              <p className="text-forza-red/50 text-[10px] uppercase tracking-wider">End</p>
              <p className="text-forza-red text-sm font-semibold mt-0.5">{formatDate(subscription.endDate)}</p>
            </div>
          </div>

          <div className="mt-4 flex items-center justify-between">
            <p className="text-forza-red/60 text-xs">
      ...
      <section className="grid grid-cols-3 gap-2.5 mb-5 animate-slide-up" style={{ animationDelay: '0.1s' }}>
        {[
          { icon: Flame, label: 'Streak', value: `${USER.streak}d`, color: 'text-forza-red/75' },
          ...
          <div key={label} className="bg-forza-white border border-forza-red/15 rounded-2xl p-3.5 text-center">
            <Icon size={18} className={`${color} mx-auto`} />
            <p className="text-forza-red text-lg font-bold mt-1.5">{value}</p>
            <p className="text-forza-red/50 text-[10px] font-medium">{label}</p>
          </div>
        ))}
      </section>

      {/* Crowd meter */}
      <section className="bg-forza-white border border-forza-red/15 rounded-2xl p-4 mb-5 animate-slide-up" style={{ animationDelay: '0.15s' }}>
        <div className="flex justify-between items-center mb-3">
          <p className="text-forza-red text-sm font-semibold">Crowd Meter</p>
          <span className={`text-xs font-bold ${crowdColor}`}>{GYM_INFO.crowdLevel}%</span>
        </div>
        <div className="h-2.5 bg-forza-red/5 rounded-full overflow-hidden">
          <div
        ...
        <p className="text-forza-red/50 text-[11px] mt-2.5">
      ...
      <section className="mb-5 animate-slide-up" style={{ animationDelay: '0.2s' }}>
        <h2 className="text-forza-red text-sm font-semibold mb-3">Quick Actions</h2>
        <div className="grid grid-cols-2 gap-2.5">
          {[
            { label: 'Freeze Plan', tab: 'membership' as Tab, icon: Snowflake, color: 'text-forza-red/75' },
            ...
            { label: 'Gym Events', tab: 'events' as Tab, icon: Tv, color: 'text-forza-red/75' },
          ].map(({ label, tab, icon: Icon, color }) => (
            <button
              key={label}
              onClick={() => onNavigate(tab)}
              className="bg-forza-white border border-forza-red/15 rounded-2xl p-4 text-left active:scale-[0.98] transition-transform hover:border-forza-red/25"
            >
              <Icon size={20} className={color} />
              <p className="text-forza-red text-sm font-semibold mt-2">{label}</p>
            </button>
          ))}
        </div>
      </section>
      ...
      {activeAddons.length > 0 && (
        <section className="mb-5 animate-slide-up" style={{ animationDelay: '0.25s' }}>
          <h2 className="text-forza-red text-sm font-semibold mb-2">Active Add-ons</h2>
      ...
      <section className="animate-slide-up" style={{ animationDelay: '0.3s' }}>
        <div className="flex justify-between items-center mb-3">
          <h2 className="text-forza-red text-sm font-semibold">What's On</h2>
        ...
        {(registered.length > 0 ? registered : upcoming).slice(0, 2).map((ev) => (
          <div key={ev.id} className="bg-forza-white border border-forza-red/15 rounded-2xl p-4 mb-2.5 flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl bg-forza-red/5 flex items-center justify-center shrink-0">
              <EventIcon event={ev} />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-forza-red text-sm font-medium truncate">{ev.title}</p>
              <p className="text-forza-red/50 text-[11px] mt-0.5">{formatDate(ev.date)} · {ev.time}</p>
            </div>
      ...
      <footer className="flex items-center gap-2 text-forza-red/50 mt-5 pb-2">
        <MapPin size={12} />

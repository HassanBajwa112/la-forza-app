    ...
    <ScreenLayout>
      ...
      <section className="grid grid-cols-2 gap-3 mb-6">
        <div className="surface-card text-center py-5">
          <p className="stat-value text-3xl">{activeDays}</p>
          <p className="label-caps-muted mt-2">Training Days</p>
        </div>
        <div className="surface-card text-center py-5">
          <p className="stat-value text-3xl">{totalSets}</p>
          <p className="label-caps-muted mt-2">Workout Sets</p>
        </div>
      </section>
            ...
            <div key={day.day}>
              <button
                ...
                className={`w-full flex items-center justify-between p-4 rounded-2xl border transition-all ${
                  isToday
                    ? 'bg-forza-elevated border-forza-red/30'
                    : isOpen
                    ? 'bg-forza-surface border-forza-red/20'
                    : 'bg-forza-surface border-forza-border'
                }`}
              >
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-display font-bold text-sm ${
                    isToday ? 'bg-forza-red text-forza-white' : 'bg-forza-elevated text-forza-muted'
                  }`}>
                    {day.shortDay}
                  </div>
                  <div className="text-left">
                    <p className="text-forza-white text-sm font-semibold">
                      {day.day}
              ...
              {isOpen && (
                <div className="mt-2 ml-4 pl-4 border-l-2 border-forza-border space-y-2 animate-slide-up">
                    ...
                    <div key={set.id} className="bg-forza-elevated border border-forza-border rounded-xl overflow-hidden">
                      ...
                      >
                        ...
                        <div className="flex-1 text-left">
                          <p className="text-forza-white text-sm font-semibold">{set.name}</p>
                          <p className="text-forza-muted text-[10px]">{set.muscleGroup} · {set.exercises.length} exercises</p>
                        </div>
                        <button
                          onClick={(e) => { e.stopPropagation(); removeSetFromDay(dayIndex, set.id); }}
                          className="p-1.5 text-forza-muted hover:text-forza-red"
                        >
                          <X size={16} />
                        </button>
                      </button>
                        ...
                        <div className="px-4 pb-3 border-t border-forza-border">
                          <ul className="mt-2 space-y-1.5">
                            {set.exercises.map((ex, i) => (
                              <li key={ex} className="flex items-center gap-2.5 text-[11px] text-forza-white/80">
                                <span className="w-5 h-5 rounded-md bg-forza-surface flex items-center justify-center text-forza-muted text-[10px] font-medium">
                                  {i + 1}
                                </span>
                                {ex}
                              </li>
                  ...
                  <p className="text-forza-muted text-[10px] font-medium pt-1">Add workout:</p>
                  <div className="flex flex-wrap gap-2 pb-2">
                    {WORKOUT_TEMPLATES.filter((t) => !day.sets.find((s) => s.id === t.id)).map((template) => (
                      <button
                        key={template.id}
                        onClick={() => addSetToDay(dayIndex, template)}
                        className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-forza-border text-[11px] text-forza-white/85 font-medium hover:border-forza-red/40 active:scale-95 transition-all"
                      >
                        <Plus size={12} />
        ...
        <div className="grid grid-cols-2 gap-2.5">
          {WORKOUT_TEMPLATES.map((t) => (
            <div key={t.id} className="info-card py-3">
              ...
              <p className="text-forza-muted text-[10px]">{t.exercises.slice(0, 3).join(', ')}...</p>
            </div>
          ))}
        </div>
      </section>
